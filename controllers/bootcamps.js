...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
    ...
    if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(new ErrorRespose(`User ${req.params.id} is not authorized to update this bootcamp`, 401))
    }
...
exports.deleteBootcamp = asyncHandler(async (req, res, next) => {
    ...
    // Make sure user is bootcamp owner
    if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(new ErrorRespose(`User ${req.params.id} is not authorized to delete this bootcamp`, 401))
    }
...
exports.getBootcampsInRadius = asyncHandler(async (req, res, next) => {
    ...
    // Divide distance by radius of the earth (r = 6378km)
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
    ...
    // Make sure user is bootcamp owner
    if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(new ErrorRespose(`User ${req.params.id} is not authorized to upload this bootcamp`, 401))
    }    
